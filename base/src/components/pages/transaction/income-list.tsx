...
export default function IncomeList() {
    ...
    const { data: resData, isLoading: loading, mutate } = useSWR(
        [filter, '/api/incomes'],
        () => axios.get('/api/incomes', {params: filter}).then((res) => res.data),
        {revalidateOnFocus: false});

    const { data: resCategories } = useSWR(
        [filter, '/api/categories'],
        () => axios.get('/api/categories', {params: {sort: DefaultSort.name.value, type: 'income'}}).then(res => res.data),
        {revalidateOnFocus: false});

    useBreadcrumbs([
        { title: 'Incomes', href: '/app/incomes'}
    ]);
